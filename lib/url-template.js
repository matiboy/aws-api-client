...
exports.uritemplate = function () {
  ...
  */
	...
	Expression.build = function (ops, vars) {
		...
		switch (ops) {
			case '':
				conf = simpleConf;break;
			case '+':
				conf = reservedConf;break;
			case '#':
				conf = fragmentConf;break;
			case ';':
				conf = pathParamConf;break;
			case '?':
				conf = formParamConf;break;
			case '&':
				conf = formContinueConf;break;
			case '/':
				conf = pathHierarchyConf;break;
			case '.':
				conf = labelConf;break;
			default:
  ...
  */
	...
	var match2varspec = function match2varspec(m) {
		var name = m[1];
		var expl = m[3];
		var part = m[4];
	...
	var match2expression = function match2expression(m) {
		var expr = m[0];
		var ops = m[2] || '';
	...
	var pushLiteralSubstr = function pushLiteralSubstr(set, src, from, to) {
		if (from < to) {
